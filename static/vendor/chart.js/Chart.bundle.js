...
function defineLocale (name, config) {
    if (config !== null) {
        ...
        // backwards compat for search: also set the locale
...
function dayOfYearFromWeekInfo(config) {
    ...
    if (w.GG != null || w.W != null || w.E != null) {
        ...
        // how we interpret search (local, utc, fixed offset). So create
        // a search version of current config (take local/utc/offset flags, and
        // create search).
...
module.exports = function(Chart) {
	...
	Chart.layoutService = {
		...
		update: function(chart, width, height) {
			...
			// Essentially we search have any number of boxes on each of the 4 sides.
...
module.exports = function(Chart) {
	...
	Chart.Scale = Element.extend({
		...
		update: function(maxWidth, maxHeight, margins) {
			...
			// Ticks - `this.ticks` is search DEPRECATED!
			// Internal ticks are search stored as objects in the PRIVATE `this._ticks` member
...
module.exports = {
	...
	generators: {
		...
		logarithmic: function(generationOptions, dataRange) {
			...
			// the axis area. For search, we say that the minimum tick spacing in pixels must be 50
